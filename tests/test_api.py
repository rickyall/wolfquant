...
import tempfile
import time
from wolfquant.future import ApiStruct, MdApi
...
class MyMdApi(MdApi):
    def __init__(self, brokerID, userID, password, instrumentIDs):
        self.requestID = 0
        self.brokerID = str.encode(brokerID)
        self.userID = str.encode(userID)
        self.password = str.encode(password)
        self.instrumentIDs = [str.encode(s) for s in instrumentIDs]
        self.Create()
...
if __name__ == '__main__':
    ...
    mdapi = MyMdApi(user_info['brokerID'],
                    user_info['userID'],
                    user_info['password'],
                    ['RB1801'])
    mdapi.RegisterFront(user_info['register_front'])
    mdapi.Init()
